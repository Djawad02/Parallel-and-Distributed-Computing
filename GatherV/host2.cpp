...
int main(int argc, char** argv) {
	...
	int numElem = rand() % (200 - 20 + 1) + 20;
	int *arr1 = (int*)malloc(sizeof(int) * numElem);
	int *arr2 = (int*)malloc(sizeof(int) * numElem);
	int SIZE = 10;
	...
	cl_mem original_buffer2 = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int)*numElem, NULL, NULL);
	...
	clEnqueueWriteBuffer(commandQueue, original_buffer, CL_TRUE, 0, sizeof(int) * numElem, arr2, 0, NULL, NULL);
	...
	// Create kernel
	cl_kernel kernel = clCreateKernel(program, "calCulatingDotProdct", &retErr);
	// Set the kernel arguments
	// Set the kernel arguments
	clSetKernelArg(kernel, 0, sizeof(cl_mem), (void*)&original_buffer);
	clSetKernelArg(kernel, 1, sizeof(cl_mem), (void*)&original_buffer2);
	clSetKernelArg(kernel, 2, sizeof(cl_mem), (void*)&result_nuffer);
	clSetKernelArg(kernel, 3, sizeof(cl_mem), (void*)&subArr_buffer); 
	clSetKernelArg(kernel, 4, sizeof(cl_mem), (void*)&offsett_buffer); 
	...
	int* local_product = (int*)malloc(sizeof(int) * SIZE);;
	clEnqueueReadBuffer(commandQueue, result_nuffer, CL_TRUE, 0, sizeof(int) * SIZE, local_product, 0, NULL, NULL);
	/*printf("DDDDD %d", result_nuffer);*/
	// Calculate the global sum
	int global_p = 0;
	printf("\n\t\t\t\t\t\t\t***In Host*** \n");
	for (int i = 0; i < SIZE; i++) {
		printf("local prodct: %d\n", local_product[i]);
		global_p += local_product[i];
	}
